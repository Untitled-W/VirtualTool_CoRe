...
function addTool2(worldDict, toolverts, pos) {{
    if (typeof(toolverts) === 'undefined') return worldDict
    var ppolys = []
    for (var i = 0; i < toolverts.length; i++) {{
        var tpol = []
...
function getGWPathAndRotPlacementTwoTools(worldDict, toolverts1, pos1, toolverts2, pos2, maxtime, stepSize, noiseDict, returnNewWorld) {{
    var w = addTool(worldDict, toolverts1, pos1);
    w = addTool2(w, toolverts2, pos2);
    return getGWPathAndRot(w, maxtime, stepSize, noiseDict, returnNewWorld);
}}
